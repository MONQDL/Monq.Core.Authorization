...
namespace Monq.Core.Authorization.Helpers
{
    ...
    internal static class PacketRepository
    {
        ...
        /// <param name="userspaceId">Userspace identifier.</param>
        /// <returns></returns>
        public static bool NotExistsOrExpired(long userId, string? userspaceId)
        {
            var item = _packets.TryGetValue(new CachedUser { UserId = userId, UserspaceId = userspaceId }, out var cachedItem) ? cachedItem : null;
        ...
        public static void Set(long userId, string? userspaceId, PacketViewModel grant, TimeSpan? cacheTimeout = default)
            => _packets[new CachedUser { UserId = userId, UserspaceId = userspaceId }] = new CachedPacket { Packets = new[] { grant }, AddedAt = DateTimeOffset.UtcNow, CacheTimeout = cacheTimeout };
        ...
        /// <param name="cacheTimeout">If set, the packet will be cached for <paramref name="cacheTimeout"/>.</param>
        public static void Set(long userId, string? userspaceId, IEnumerable<PacketViewModel> grants, in TimeSpan? cacheTimeout = default)
            => _packets[new CachedUser { UserId = userId, UserspaceId = userspaceId }] = new CachedPacket { Packets = grants, AddedAt = DateTimeOffset.UtcNow, CacheTimeout = cacheTimeout };
    }
